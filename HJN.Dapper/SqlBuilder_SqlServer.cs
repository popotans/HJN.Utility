...
{
    class SqlBuilder_SqlServer : ISqlBuilder
    {
        ...
        public string Insert<T>(T t) where T : BaseEntity
        {
            ...
            sb.AppendFormat("INSERT INTO {0}{1}{2}(", ColumnBeginChar, t.TableName, ColumnEndChar);
            int i = 0;
        ...
        public string Delete<T>(T t) where T : BaseEntity
        {
            StringBuilder sb = new StringBuilder();
            PropertyInfo[] propertiesArr = t.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance);
            sb.AppendFormat("DELETE FROM {0}{1}{2} where ", ColumnBeginChar, t.TableName, ColumnEndChar);
            int i = 0;
            foreach (string primary in t.PrimaryKeys)
            {
                // paramValsDic[primary].BuildValueFormat()
